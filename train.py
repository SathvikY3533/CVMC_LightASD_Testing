...
def main():
    ...
    parser.add_argument('--savePath', type=str, default="/content/drive/MyDrive/CVMC_UTD_Summer_Internship_2024/LightASD_Testing/")
    ...
    while epoch <= args.maxEpoch:
        ...
        if epoch % args.testInterval == 0:
            ...
            mAP = s.evaluate_network(epoch=epoch, loader=valLoader, **vars(args))
            mAPs.append(mAP)
            epochs_with_mAP.append(epoch)
            print(time.strftime("%Y-%m-%d %H:%M:%S"), "%d epoch, mAP %2.2f%%, bestmAP %2.2f%%" % (epoch, mAP, max(mAPs)))
            scoreFile.write("%d epoch, LR %f, LOSS %f, mAP %2.2f%%, bestmAP %2.2f%%\n" % (epoch, lr, loss, mAP, max(mAPs)))
            scoreFile.flush()
    ...
    plt.subplot(1, 2, 1)
    plt.plot(epochs_with_mAP, mAPs, label='Accuracy(mAP)')
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.legend()
